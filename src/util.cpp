...
const char * const BITCOIN_CONF_FILENAME = "lgc.conf";
const char * const BITCOIN_PID_FILENAME = "lgcd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "lgc" is a composite category enabling all Lgc-related debug output
                if(ptrCategory->count(std::string("lgc"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LgcCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LgcCore
    // Mac: ~/Library/Application Support/LgcCore
    // Unix: ~/.lgccore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LgcCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LgcCore";
#else
    // Unix
    return pathRet / ".lgccore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty lgc.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override lgc.conf
